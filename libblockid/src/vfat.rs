    ...
    ) -> Result<() ,Box<dyn std::error::Error>> 
{
    ...
    let mut vol_serno: FsUuid = FsUuid::VolumeId32([0u8; 4]);
    ...
    if ms.ms_fat_length != 0 {
        ...
        if ms.ms_ext_boot_sign == 0x28 || ms.ms_ext_boot_sign == 0x29 {
            let mut buf = [0u8; 4];
            LittleEndian::write_u32(&mut buf, ms.ms_serno);
            vol_serno = FsUuid::VolumeId32(buf);
        }
